 ...
 *  微信公众平台SDK FOR TP
 *  @author  rakiy 
 *  @link https://github.com/rakiy/wechat
 *  @version 1.0
 *  usage:
 *   $options = array(
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function setTMIndustry($id1,$id2=''){
		...
		var_dump($this->uuu);
		var_dump(self::API_URL_PREFIX.self::TEMPLATE_SET_INDUSTRY_URL.'access_token='.$this->access_token);
		var_dump(self::json_encode($data));
		if($result){
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	 ...
	 * @return boolean|array   返回数组请参看 微信卡券接口文档 的json格式
	 */
	public function getCardLocations($offset=0,$count=50) {
		$data=array(
			'begin'=>$offset,
			'limit'=>$count
		);
		if (!$this->access_token && !$this->checkAuth()) return false;
		$result = $this->http_post(self::API_BASE_URL_PREFIX . self::POI_GET_LIST . 'access_token=' . $this->access_token, self::json_encode($data));
		if ($result) {
			$json = json_decode($result, true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
